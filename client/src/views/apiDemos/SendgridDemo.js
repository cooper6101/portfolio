...
import { sendSendgridEmail } from 'actions/apiActions';
...
const SendgridDemo = () => {
  ...
  const sendgridEmailObject = useSelector((state) => state.sendgridEmailObject);
  const { msg, loading, error } = sendgridEmailObject;
  ...
  const submitHandler = (e) => {
    ...
    if (robotCheck) {
      ...
      dispatch(sendSendgridEmail(firstName, lastName, email, message));
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          'url(' + require('assets/img/gmailImg.jpeg').default + ')'
      }}
      id='sendgrid-api-demo'
    >
      <Container>
        <Row>
          <Col md='5'>
            <h2 className='title'>Sendgrid Demo</h2>
            <h4 className='description'>
              Use this form to send yourself an email! This form utilizes
              Sendgrid's API to send emails to a single user.
            </h4>
            ...
            <Card className='card-contact card-raised'>
              <Form id='contact-form1' onSubmit={submitHandler}>
                <CardHeader className='text-center'>
                  <CardTitle tag='h4'>Email Yourself</CardTitle>
                </CardHeader>
                <CardBody>
                  {msg && (
